...
export function AlbumCard({ songs, album, artist, className, isPinned }: Props) {
  ...
  return (
    <div
      className={cn(
        "group flex flex-col items-stretch gap-1 text-sm transition-all hover:bg-white/3 rounded-sm p-2 w-full h-fit cursor-pointer",
        className
      )}
    >
      <div className="relative aspect-square rounded-sm overflow-hidden flex-none w-full bg-typography-gray/5 group-hover:shadow-lg">
        ...
        >
          <AlbumPinBtn
            className="transition-[translate,opacity] duration-[400ms] opacity-0 group-hover:-translate-y-10 group-hover:opacity-100 absolute ease-out -bottom-[calc(22px)] left-4 size-10 bg-[rgba(26,26,26,.9)] hover:bg-[rgba(51,51,51,.9)]"
            isInitiallyPinned={isPinned}
            album={{ ...album, songs, artist }}
          />
          <motion.div
            onClick={handleClick}
            whileTap={{
            ...
            className={cn(
              ...
              isPaused && currentQueueId === queueId && "-translate-y-10 opacity-100"
            )}
          >
            {!isPaused && currentQueueId === queueId ? (
              <FaPause size={20} />
      ...
      <div className="flex flex-col gap-0.5 text-base flex-1">
        ...
        <p className="text-typography-gray text-sm">{album.releaseYear}</p>
      </div>
    </div>
  );
}
